...
STATUS_ID=$(gh api "repos/${REPO}/issues/${ENTRY_ID}/comments" \
  --jq '[.[] | select(.body | startswith("🤖 **Web app test"))] | first | .id // empty')
```
...
STATUS_ID=$(gh api "repos/${REPO}/issues/${ENTRY_ID}/comments" \
  --jq '[.[] | select(.body | startswith("🤖 **Web app test"))] | first | .id')
```

**2b. `STATUS_ID` is non-empty → adopt it.** Another dispatch already posted one. Do not post a second; continue with that id.

**`first`, not `last`.** The query deliberately takes the *oldest* matching comment. With `last`, a PR that has been triggered more than once gives each run a different comment — run 2 adopts whatever run 1 most recently created — so the runs interleave and the thread keeps growing. Taking the first makes every run converge on the same comment no matter how many already exist.

**Re-triggering is normal.** Removing and re-adding the label fires the webhook again, so a PR routinely accumulates runs. The status comment is per-PR, not per-run: a later run overwrites the earlier one's state rather than starting a fresh comment. If a status line looks out of order — `Ready` appearing after `Executing` — two runs are writing to different comments, which is exactly what `first` prevents.
...
When either flag is set, read and follow `skills/apply-proposed-cases/SKILL.md` and do what it says instead of the default Phase 1–3 sequence. It covers both halves: proposing cases for review without opening a browser, and applying approved cases to the sheet before running the enlarged suite.

**When neither flag is set — the default — skip this section.** A default run does its coverage work in **Phase 1.5** instead: it settles coverage before executing, pausing for an answer when the PR adds behaviour the sheet does not cover. `--apply-cases` enters at step 5 of that skill to apply an answer already given.
...
## Phase 1.5 — Settle Coverage Before Executing
...
5. **On the next trigger** — the approval label, or a re-run after a box is ticked — read the answer per `skills/apply-proposed-cases/SKILL.md`:
   ...
   - **Yes** → append the rows, commit the sheet, and execute the enlarged suite (`<n>+<k>` cases).
   - **No, or a re-trigger with neither box ticked** → set the status to `Declined`, then execute the existing `<n>` cases and note the declined proposal in the report. A decline still produces a full run and a full report — the only difference from an approval is that the sheet is untouched.

   **Never leave a PR paused with no path forward.** If the proposal has already been consumed (`<!-- wat:consumed -->` present) and the sheet still lacks the cases, treat that as a decline and run the existing cases rather than re-proposing or stopping again. Re-proposing something the author has already answered is noise.

**Why the run stops rather than testing while it waits.** A run that executes 52 cases and then asks about 3 more has to be repeated in full to cover them — twice the budget for one answer. Stopping costs one cheap run, and the run that follows tests the complete suite once.

This applies to a default run. `--propose-only` behaves identically by design, and `--apply-cases` skips straight to step 5.
